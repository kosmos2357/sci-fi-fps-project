...
@onready var mesh_node = $LighMesh
...
# WARNING: Error on nul;l instance create timer
# -- Public API Methods

func power_on():

	is_powered = true
	if pulse_pattern == PulsePattern.FLICKER_RANDOM:
		_random_flicker_loop()
	if is_broken and is_powered:
		...
		# My hanlding of emission = true should be more at the top. So pulsate doesnt have to handle
		# The logic as well
		light_node.light_energy = light_energy
		mesh_node.get_active_material(0).emission_enabled = true
...
func _random_flicker_loop():
	# This loop only runs if the light is powered and set to this pattern
	if is_powered and pulse_pattern == PulsePattern.FLICKER_RANDOM:
		# Toggle the light state
		light_node.visible = not light_node.visible
		mesh_node.get_active_material(0).emission_enabled = light_node.visible

		# Set a new timer to call this function again after a random delay
		var delay = randf_range(0.05, 2)
		get_tree().create_timer(delay).timeout.connect(_random_flicker_loop)
...
"""
On the recurring null timer error
fix put entity initialization in call defferred
make sure signal connections are made only once for signal error
